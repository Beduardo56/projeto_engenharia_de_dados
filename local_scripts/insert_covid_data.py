...
rdsdb = connect(host=os.environ["RDS_HOST"], user=os.environ["RDS_USER"], password=os.environ["RDS_PASSWORD"], database="covid")
print('Conectado')
cursor = rdsdb.cursor()
initial_query = """CREATE TABLE IF NOT EXISTS covid_timeline(measured_at datetime, country_code varchar(10), latitude float, longitude float, acumulated_confirmed_cases integer, acumulated_deaths interger)"""
cursor.execute(initial_query)

if __name__ == "__main__":
    for data in get_confirmed_cases_among_time():
